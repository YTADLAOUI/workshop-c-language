...
{
  int i, nombreProduites, choix,prixach;
  system("cls");
  printf("\n");
  ...
  for (i = 0; i < nombreProduites; i++)
  {
    ...
    scanf("%d",&prixach);
        if(prixach<0){
    Produits[nombresDesProduits].prix=prixach;
    }else 
    goto rply;
    printf("\n");
     nombresDesProduits++;
...
venteProduits()
{
    ...
    for(i=0;i<nombresDesProduits;i++)
    {
      ...
      printf("le vent de prduit TTC  : %.2f dhs \n\n",achter[nombresDesAcheter].prix);
...
// les statistiques
        void statistiqueVente()
    {
		...
		printf("le total des produits vendus :  %.2f\n\n",total);
        	...
        	moyenne = total/nombresDesAcheter;
