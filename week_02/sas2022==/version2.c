...
bool produitExist(char codeExiste[10]){
int i; 
for(i=0;i<nombresDesProduits;i++){
	if(strcmp(Produits[i].code,codeExiste)==0);
	return true;
}return false;
}
...
void ajouterUnProduit()
{
  ...
  scanf("%f", &Produits[nombresDesProduits].prix);
	...
	puts ("code est deja existe veuillez entre un nauveau code ou aller a alimenter le stock : ");
